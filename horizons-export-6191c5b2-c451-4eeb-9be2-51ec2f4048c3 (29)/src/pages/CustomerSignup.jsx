...
const CustomerSignup = () => {
  ...
  useEffect(() => {
    const fetchLocationData = async () => {
      ...
      setError(null);
      if (!locationId) {
        setError("No location specified in the link.");
      ...
      try {
        ...
        if (dbError || !data) {
          throw new Error("Invalid signup link. Please scan a valid QR code.");
        }
  ...
  const handleSubmit = async (e) => {
    ...
    // After successful signup, sign in to create a session
    ...
    // On successful sign-in, redirect to the auth handler
    toast({
      title: "Welcome! 🎉",
  ...
  const renderContent = () => {
    ...
    if (error) {
      return (
        <div className="text-center text-red-400 glass-effect p-8 rounded-lg">
          <AlertTriangle className="w-12 h-12 mx-auto mb-4 text-red-500" />
          <h2 className="text-xl font-bold mb-2 text-white">Signup Error</h2>
          <p>{error}</p>
        </div>
      );
    }
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Join a digital loyalty program and start earning rewards today!" />
      </Helmet>
