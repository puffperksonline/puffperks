...
const RewardManager = ({
  ...
  const fetchRewards = async () => {
    ...
    const {
      data,
      error: fetchError
    } = await supabase.from('rewards').select('*').eq('store_id', storeId).order('stamps_required');
    if (fetchError) {
  ...
  const handleSaveReward = async e => {
    ...
    if (error) {
      ...
      toast({
        title: `Reward ${editingReward ? 'updated' : 'added'}!`,
        description: 'Your rewards list has been updated.'
      });
      await fetchRewards();
  ...
  return <>
      ...
      delay: 0.5
        ...
        <div className="space-y-3">
          {rewards && rewards.length > 0 ? rewards.map(reward => <div key={reward.id} className={`flex items-center justify-between p-3 bg-gray-800/50 rounded-lg ${!reward.is_active ? 'opacity-50' : ''}`}>
                <div>
              ...
              </div>) : <p className="text-gray-400 text-center py-4">No rewards yet. Add one to get started!</p>}
        </div>
      </motion.div>
        ...
        <DialogContent className="glass-effect">
          ...
          <form onSubmit={handleSaveReward} className="space-y-4">
              <div>
                  ...
                  <Input id="stamps_required" name="stamps_required" type="number" defaultValue={editingReward?.stamps_required ?? ''} required className="bg-gray-800/50 border-gray-600 mt-1" />
              </div>
              <div>
                  <Label htmlFor="description" className="text-white">Description</Label>
                  <Input id="description" name="description" type="text" defaultValue={editingReward?.description ?? ''} required className="bg-gray-800/50 border-gray-600 mt-1" />
              </div>
              <div className="flex items-center space-x-2">
                <Switch id="is_active" name="is_active" defaultChecked={editingReward ? editingReward.is_active : true} />
