...
export const QRCodeCard = ({ selectedLocation, storeName, onCardDesignSave, rewards }) => {
  ...
  const qrCodeUrl = `${window.location.origin}/customer/signup/${selectedLocation.id}`;
  ...
  const handleDownloadQr = (bgColor, fgColor, filetype = 'png') => {
    ...
    qrcode.toDataURL(qrCodeUrl, {
      ...
      const safeStoreName = selectedLocation.name ? selectedLocation.name.replace(/\s+/g, '-') : 'location';
      const fileName = `${safeStoreName}-qr-code${isTransparent ? `-${colorName}-transparent` : ''}.${filetype}`;
  ...
  return (
    ...
    >
      <h2 className="text-xl font-semibold text-white mb-4 flex items-center">
        ...
        Customer Signup QR Code for: <span className="text-purple-300 ml-2">{selectedLocation.name}</span>
      </h2>
