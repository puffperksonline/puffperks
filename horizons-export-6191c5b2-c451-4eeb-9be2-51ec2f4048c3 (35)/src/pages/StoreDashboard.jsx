...
const ProBanner = ({ onUpgradeClick }) => {
  return (
    <motion.div
      ...
      className="mb-8 p-4 bg-gradient-to-r from-purple-600 to-pink-600 rounded-lg text-white flex items-center justify-between shadow-lg"
    >
      <div>
        <h3 className="font-bold">Unlock Pro Features!</h3>
        <p className="text-sm">Get customer segmentation, advanced analytics, and more.</p>
      </div>
      <Button onClick={onUpgradeClick} variant="secondary" className="bg-white/20 hover:bg-white/30">
        Upgrade Now
      </Button>
    </motion.div>
  );
...
const StoreDashboard = () => {
  ...
  const handleUpgradeClick = () => {
    const upgradeUrl = `https://buy.stripe.com/bJeaEW4e59gWdfa9XT5kk01?prefilled_email=${encodeURIComponent(user?.email || '')}`;
    ...
    <>
      ...
      >
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <ProBanner onUpgradeClick={handleUpgradeClick} />
