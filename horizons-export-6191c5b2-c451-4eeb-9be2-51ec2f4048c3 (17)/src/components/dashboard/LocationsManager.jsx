...
const LocationForm = ({ storeId, onSave, editingLocation, setEditingLocation, isFirstLocation }) => {
  ...
  const handleGoToSettings = () => {
      navigate('/store/settings/locations');
  }

  if (isFirstLocation) {
      return (
          <div className="text-center p-6 border border-dashed border-gray-600 rounded-lg">
              <h3 className="text-lg font-semibold">Your first location is added!</h3>
              <p className="text-gray-400 mt-2 mb-4">You can now generate QR codes from your dashboard.</p>
              <Button onClick={handleGoToSettings}>Go to Settings to manage it</Button>
          </div>
      )
  }
...
export const LocationsManager = ({ store, onUpdate }) => {
  ...
  return (
    <div className="space-y-6">
      <LocationForm storeId={store?.id} onSave={handleSave} editingLocation={editingLocation} setEditingLocation={setEditingLocation} isFirstLocation={false}/>
