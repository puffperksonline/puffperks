...
export const TrialCard = ({ trialEndsAt, paymentLink }) => {
  const { user } = useAuth();
  const upgradeUrl = paymentLink ? `${paymentLink}?prefilled_email=${encodeURIComponent(user?.email || '')}` : `https://buy.stripe.com/6oUfZgdOF64K7UQ9XT5kk01?prefilled_email=${encodeURIComponent(user?.email || '')}`;
