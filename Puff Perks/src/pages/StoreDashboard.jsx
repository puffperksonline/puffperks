...
import NoticeBoardCard from '@/components/dashboard/NoticeBoardCard.jsx';
...
const StoreDashboard = () => {
  ...
  const fetchStoreData = useCallback(async () => {
    ...
    try {
        ...
        .from('notices')
        .select('*')
        .or(`show_to_all_stores.eq.true,target_store_ids.cs.{${store.id}}`)
        .order('created_at', { ascending: false });
        
        if (noticeError) {
          console.error("Could not fetch notices:", noticeError.message);
          setNotices([]);
        } else if (noticeData) {
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="hidden lg:grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="space-y-8">
              ...
              {notices.map(notice => <NoticeBoardCard key={notice.id} notice={notice} />)}
            </div>
          </div>
