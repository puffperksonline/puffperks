...
const ManualStampModal = ({ isOpen, onClose, storeId, locationId, onUpdate }) => {
  ...
  const handleFindCustomer = async () => {
    ...
    if (found) {
      const { data: card } = await supabase.from('loyalty_cards').select('id').eq('customer_id', found.id).eq('location_id', locationId).limit(1).single();
      if(card) {
        setCustomerFound({ ...found, loyalty_card_id: card.id });
        toast({ title: 'Customer Found!', description: `${found.full_name} is ready for stamps.`});
      } else {
        toast({ title: "No loyalty card found for this customer at this location.", description: "They may need to sign up using this location's QR code.", variant: "destructive" });
      }
    } else {
  ...
  const handleAddStamps = async () => {
    ...
    for (let i = 0; i < stamps; i++) {
        promises.push(supabase.functions.invoke('add-stamp-manually', {
            body: { loyalty_card_id: customerFound.loyalty_card_id, storeId: storeId, locationId: locationId },
        }));
    }
