...
const AuthRedirect = () => {
  ...
  React.useEffect(() => {
    ...
    const checkUserRoleAndRedirect = async () => {
      ...
      const { data: loyaltyCard, error: cardError } = await supabase.from('loyalty_cards').select('id').eq('customer_id', user.id).maybeSingle();
      if (loyaltyCard) {
        navigate(`/customer/card/${loyaltyCard.id}`);
        return;
      }
      
      // If neither, there might be a delay in data creation. Retry or redirect.
      // This case should be rare with the new DB function.
