...
const AdminDashboard = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (storesError) throw new Error(`Stores fetch failed: ${storesError.message}`);
      ...
      if (customersError) throw new Error(`Customers count failed: ${customersError.message}`);
      ...
      if (noticesError) throw new Error(`Notices fetch failed: ${noticesError.message}`);
      ...
      toast({
        ...
        description: `Could not fetch admin data: ${fetchError.message}`,
        variant: 'destructive',
      });
    }
  }, [toast]);
  ...
  useEffect(() => {
    const checkAdminStatus = async () => {
      if (user) {
        ...
        if (userError || !userData?.raw_user_meta_data?.is_super_admin) {
  ...
  if (loading) {
    return <div className="min-h-screen flex items-center justify-center bg-gray-900 text-white">Verifying Access & Loading Portal...</div>;
  }
              ...
              <tbody>
                {stores.map(store => (
                  <tr key={store.id} className="border-b border-gray-800 hover:bg-gray-700/50">
                    ...
                    <td className="p-3 text-indigo-400">{store.owner_id?.email || 'N/A'}</td>
